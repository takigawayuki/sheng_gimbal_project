...
static uint8_t x_v2_uart1_tx_buf[32];
static uint8_t x_v2_uart3_tx_buf[32];
static uint8_t x_v2_uart6_tx_buf[32];

static uint8_t *X_V2_GetTxBuf(UART_HandleTypeDef *huart)
{
  if (huart == &huart1) {
    return x_v2_uart1_tx_buf;
  }

  if (huart == &huart3) {
    return x_v2_uart3_tx_buf;
  }

  if (huart == &huart6) {
    return x_v2_uart6_tx_buf;
  }

  return NULL;
}

static void X_V2_Transmit_UART_DMA(UART_HandleTypeDef *huart, uint8_t *cmd, uint16_t len)
{
...
void X_V2_Vel_Control_UART(UART_HandleTypeDef *huart, uint8_t addr, uint8_t dir, uint16_t acc, float vel, bool snF)
{
  ...
  X_V2_Transmit_UART_DMA(huart, cmd, 9);
}
...
void X_V2_Traj_Pos_Control_UART(UART_HandleTypeDef *huart, uint8_t addr, uint8_t dir, uint16_t acc, uint16_t dec, float vel, float pos, uint8_t raf, bool snF)
{
  uint8_t *cmd = X_V2_GetTxBuf(huart);
  uint16_t v = (uint16_t)ABS(vel * 10.0f);
  uint32_t p = (uint32_t)ABS(pos * 10.0f);
  ...
  X_V2_Transmit_UART_DMA(huart, cmd, 16);
}
